  ...
  return (
    <div>
      ...
      <div className="w-full flex justify-center">
        <nav className={`fixed top-0 w-auto mt-7 ${activeSection === 'welcome' ? 'bg-transparent' : 'bg-gradient-background-image'} text-cos-off-black p-4 px-10 z-10 hidden sm:flex justify-center rounded-full`}>
            ...
            {sections.map((section) => (
              <li key={section}>
                <Link
                  ...
                  smooth={true}
                  duration={500}
                  className={`cursor-pointer font-omnes ${activeSection === section ? "border-b-2 border-black" : ""
                    }`}
                >
